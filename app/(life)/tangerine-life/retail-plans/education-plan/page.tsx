...
import { EDUCATION_PLANS } from '@/constants/life'
    ...
    <>
      <section className="relative max-container padding-container flex flex-col gap-12 md:flex-row items-start w-full mt-36">
        <div className="w-full space-y-3 flex-[2]">
          <BackWithBreadCrumbs page="Education Plan" textTheme="text-green-500" />
          <h1 className="bold-56 text-green-900">Education Plan</h1>
          <p className="regular-18 text-generic-950 !leading-[145%]">The Education Plan offers a life insurance cover that assures funding for the child/ward's education in the case of death of a parent/guardian.</p>
          <Link href="/tangerine-life/buy-policy">
            <Button variant="green" size="lg" className="mt-4">Buy a Policy</Button>
          </Link>
        </div>
        <div className="w-full flex-[3]">
          <Image src="/images/life/education-banner.png" alt="Education Plan" width={696} height={364} className="w-full object-contain" />
        </div>
      </section>

      <section className="relative max-container padding-container mt-32">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mt-10">
          {EDUCATION_PLANS.map((item: { title: string; description: string }) =>
            <AvailableRidersCard
              key={item.title}
              name={item.title}
