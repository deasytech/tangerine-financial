...
import { TANG_FLEX_PLANS } from '@/constants/life'
    ...
    <>
      <section className="relative max-container padding-container flex flex-col gap-12 md:flex-row items-start w-full mt-44">
        <div className="w-full space-y-3 flex-[2]">
          ...
          <Link href="/tangerine-life/get-quote">
            <Button variant="green" size="lg" className="mt-4">Get a Quote</Button>
          </Link>
        </div>
        <div className="w-full flex-[3]">
          <Image src="/images/life/tang-flex-banner.png" alt="Tang Flex Plan" width={696} height={364} className="w-full object-contain" />
        </div>
      </section>

      <section className="relative max-container padding-container mt-32">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mt-10">
          {TANG_FLEX_PLANS.map((item: { title: string; description: string }) =>
            <AvailableRidersCard
              key={item.title}
              name={item.title}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mt-10">
          <AvailableRidersCard
            ...
            desc={`
                ...
                <li>- Valid proof of identity of named beneficiary (on the death of the policyholder).</li>
                <li>- Police Report if death was caused by Accident, copy of Life Assured identity.</li>
                </ul>
                `}
          />
        </div>
