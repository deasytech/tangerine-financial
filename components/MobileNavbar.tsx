    ...
    <div className="grid grid-cols-1 gap-2 lg:hidden">
      <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
        ...
        <SheetContent side="left">
          ...
          <nav>
            <Link href="/" onClick={handleMenuItemClick} className="bg-orange-50 flex my-3 p-3 w-full regular-16 text-black transition-all">
              Home
            </Link>
            {NAV_LINKS.map((link: NavLink) => (
              <div key={link.key} className=" bg-orange-50">
                {link.subMenu ? (
                  <>
                    <button
                  ...
                  <Link href={link.href} onClick={handleMenuItemClick} className="bg-orange-50 flex my-3 p-3 w-full regular-16 text-black transition-all">
                    {link.label}
                  </Link>
                )}
              </div>
